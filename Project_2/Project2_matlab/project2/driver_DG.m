...
function [l2_norm]=driver_DG(integration_type,N,Ne)
u = 2; % veolcity
time_final=1; %final time 

integration_points=1; %=1 for LGL and =2 for LG - LGL is default, but feel free to experiment
%integration_type=1; %=1 is inexact and =2 is exact - used to compute how many quadrature points we need
space_method_type='dg'; % DG
...
[Mmatrix, Dmatrix] = Matrix_DSS_DG(Me,De,intma,ngl,Ne,npoin); % working on cg for now
...
Rmatrix = Mmatrix\(u*(Dmatrix-Fmatrix));
