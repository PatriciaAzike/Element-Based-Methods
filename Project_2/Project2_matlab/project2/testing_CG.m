% This code reads information from the driver file for the CG method and
...
for i=1:2 %integration_type: used to compute how many quadrature points we need.  1- inexact, 2- exact
    if i==1
        ...
        ng_points=zeros(length(N),size(NeM,2)); %number of gridpoints, N_p
        L2_inexact=zeros(length(N),size(NeM,2)); % L2 norm for inexact CG integration
        for j =1:length(N)
            for k=1:size(NeM,2)
        ...
        L2_exact=zeros(length(N),size(NeM,2)); %L2 norm for exact CG integration
        for j =1:length(N)
            for k=1:size(NeM,2)
